...
export const getRiwayat = () => {
    try {
        return async dispatch => {
            const result = await fetch('http://192.168.42.184:8000/sirius_api/riwayat_peminjaman/', {
                'method': 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
...
export const getDaftarPeminjam = () => {
    try {
        return async dispatch => {
            const result = await fetch('http://192.168.42.184:8000/sirius_api/daftar_peminjam/', {
                'method': 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
...
export const getOrderLog = (token, id) => {
    try {
        return async dispatch => {
            const result = await fetch('http://192.168.42.184:8000/sirius_api/order_log/?token=' + token + '&user=' + id, {
                'method': 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
